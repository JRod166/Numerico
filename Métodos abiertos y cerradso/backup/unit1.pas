...
begin
  OpenM:=TStringList.Create;
  CloseM:=TStringList.Create;
  OpenM.AddObject('Newton',TObject(1));
  OpenM.AddObject('Secante',TObject(2));
  OpenM.AddObject('Punto Fijo',TObject(3));
  CloseM.AddObject('Bisección',TObject(1));
  CloseM.AddObject('Falsa posición',TObject(2));
  cboAbiertos.Items.Assign(OpenM);
  cboCerrados.Items.Assign(CloseM);
  cboCerrados.ItemIndex:=0;
  cboAbiertos.ItemIndex:=0;
end;
